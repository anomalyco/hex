...
fail() {
  echo "Permanent HEX bundle validation failed: $*" >&2
  exit 1
}
...
[ "$(plist_value CFBundleIdentifier)" = com.kitlangton.Hex ] || fail "wrong bundle identifier"
...
[ -x "$bundle/Contents/MacOS/hex" ] || fail "packaged hex executable is missing"
[ ! -e "$bundle/Contents/MacOS/voice-control-watch" ] || fail "legacy executable is still packaged"
...
printf '%s\n' "$signature" | grep -Fq 'Identifier=com.kitlangton.Hex' || fail "code signature has the wrong identifier"
...
if printf '%s\n' "$entitlements" | grep -Fq 'com.apple.security.app-sandbox'; then
  fail "permanent Rust application must not be sandboxed"
fi
