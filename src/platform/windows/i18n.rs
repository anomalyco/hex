...
const TRANSLATIONS: &[(&str, [&str; 5])] = &[
    ...
    ("Hold to speak an instruction; OpenCode's reply pastes at the cursor", ["Przytrzymaj i podaj polecenie; odpowiedź OpenCode wklei się przy kursorze", "按住说出指令；OpenCode 的回复将粘贴到光标处", "押しながら指示を話すと、OpenCode の応答がカーソル位置に貼り付きます", "Halten und eine Anweisung sprechen; die OpenCode-Antwort wird am Cursor eingefügt", "Mantén pulsado y di una instrucción; la respuesta de OpenCode se pega en el cursor"]),
