...
fn dropdown_panel_layout(
    ...
    let content_height = px(DROPDOWN_PADDING * 2.0)
        + px(DROPDOWN_ROW_HEIGHT) * row_count
        + px(DROPDOWN_ROW_GAP) * row_count.saturating_sub(1);
    let desired_height = content_height.min(px(DROPDOWN_MAX_HEIGHT));
    ...
    if below >= desired_height || below >= above {
        DropdownPanelLayout {
            height: desired_height.min(below),
            top: downward_top,
        }
    } else {
...
mod tests {
    ...
    fn long_dropdown_flips_up_and_stays_inside_the_viewport() {
        ...
        assert_eq!(layout.height, px(DROPDOWN_MAX_HEIGHT));
