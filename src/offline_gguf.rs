//! The GGUF inference session owner shared by the macOS and Linux adapters.
//!
//! Both platforms load the same `transcribe-cpp` models, so the session
//! lifecycle rules that keep inference scratch memory bounded belong in one
//! place rather than per platform.
